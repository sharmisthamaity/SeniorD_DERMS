...
*/    
 ...
 TIMER0_CTL_R = 0x00000000;    // disable timer0A during setup
  ...
  TIMER0_CTL_R |= 0x00000001;   // enable timer0A 32-b, periodic, no interrupts
	...
	//ADC0_PC_R = 0x03;         // configure for 250K samples/sec
  ADC0_SSPRI_R = 0x3210;    // sequencer 0 is highest, sequencer 3 is lowest
  ADC0_ACTSS_R &= ~0x01;    // disable sample sequencer 0
	
  ADC0_EMUX_R = (ADC0_EMUX_R&0xFFFFFFF0)+0x0005; // timer trigger event on sequence 0
 //ADC0_EMUX_R = (ADC0_EMUX_R&0xFFFFFFF0); //take out
	ADC0_SSMUX0_R = 0x0001239;					//sample channels 0, 1, 2, 3, 9
 // ADC0_SSCTL0_R = 0x06;          // set flag and end       
 //ADC0_SSCTL0_R = 0x00060000;//end at 5 samples, set bit 18
ADC0_SSCTL0_R = 0x00006000;
  ...
  ADC0_ACTSS_R |= 0x01;          // enable sample sequencer 0
   ...
   //ADC0_SAC_R = ADC_SAC_AVG_2X;
	NVIC_PRI3_R = (NVIC_PRI3_R&0xFF00FFFF)|0x00200000; //priority 1
...
//data[3] is ADC3 (PE0)
//data[4] is ADC9 (PE4)
volatile uint32_t data2[1000];

void ADC0Seq0_Handler(void){
		static uint16_t index = 0;
  ADC0_ISC_R = 0x01;          // acknowledge ADC sequence 0 completion
  data[4] = (ADC0_SSFIFO0_R&0x0FFF) | 0x4000;  // 3A) read first result
	data[3] = (ADC0_SSFIFO0_R&0x0FFF) | 0x3000;  // 3A) read first result
  data[2] = (ADC0_SSFIFO0_R&0x0FFF) | 0x2000;  // 3B) read second result
	data[1] = (ADC0_SSFIFO0_R&0x0FFF) | 0x1000;  // 3A) read third result
  //data[0] = (ADC0_SSFIFO0_R&0x0FFF);  // 3B) read fourth result
/* 
	if (index <996){
	data2[index] = (ADC0_SSFIFO0_R&0x0FFF) | 0x4000;  // 3A) read first result
	data2[index + 1] = (ADC0_SSFIFO0_R&0x0FFF) | 0x3000;  // 3A) read first result
  data2[index + 2] = (ADC0_SSFIFO0_R&0x0FFF) | 0x2000;  // 3B) read second result
	data2[index + 3] = (ADC0_SSFIFO0_R&0x0FFF) | 0x1000;  // 3A) read third result
  data2[index + 4] = (ADC0_SSFIFO0_R&0x0FFF);  // 3B) read fourth result
	}
	index = index +5;
	//put in SPI fifo?
*/
	ADC_flag = 1; 
	//may not be the best way to handle sending but will work with it 4 now
}
